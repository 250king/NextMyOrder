...
export const itemRelations = relations(Item, ({ one, many }) => ({
    group: one(Group, { fields: [Item.groupId], references: [Group.id] }),
    lists: many(List),
    orders: many(Order),
}));
...
export const listRelations = relations(List, ({ one }) => ({
    ...
    item: one(Item, { fields: [List.itemId], references: [Item.id] }),
}));
...
export const transitRelations = relations(Transit, ({ many }) => ({ orders: many(Order) }));

export const deliveryRelations = relations(Delivery, ({ one, many }) => ({
    user: one(User, { fields: [Delivery.userId], references: [User.id] }),
    deliveryToOrders: many(DeliveryToOrder),
}));
...
export const paymentItemRelations = relations(PaymentItem, ({ one }) => ({
    payment: one(Payment, { fields: [PaymentItem.paymentId], references: [Payment.id] }),
}));

export const refundRequestRelations = relations(RefundRequest, ({ one }) => ({
    payment: one(Payment, { fields: [RefundRequest.paymentId], references: [Payment.id] }),
}));
...
export const deliveryToOrderRelations = relations(DeliveryToOrder, ({ one }) => ({
    delivery: one(Delivery, { fields: [DeliveryToOrder.deliveryId], references: [Delivery.id] }),
    order: one(Order, { fields: [DeliveryToOrder.orderId], references: [Order.id] }),
}));
